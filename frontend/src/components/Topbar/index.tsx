      ...
      {!inPlayerState && (
        <div className={`absolute left-1/2 -translate-x-1/2 w-full px-4 ${tier === 'wide' ? 'max-w-[440px]' : tier === 'medium' ? 'max-w-[320px]' : 'hidden'}`} ref={searchRef}>
          <div className="relative group flex items-center w-full">
            ...
            />
            {!localQuery && (
              <div className="absolute right-4 z-10 text-[10px] font-mono text-white/30 bg-white/5 border border-white/10 px-1.5 py-0.5 rounded-md opacity-100 group-focus-within:opacity-0 transition-opacity duration-200 pointer-events-none">
                ⌘K
              </div>
            )}
            {localQuery && (
