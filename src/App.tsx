  ...
  return (
    <div className="h-screen flex flex-col bg-[var(--color-bg-base)] text-[var(--color-text-primary)] overflow-hidden font-ui">
      <Topbar 
        ...
        inPlayerState={false} // Never hide search and logo, keep topbar consistent
          ...
          <AnimatePresence>
            {inPlayerState && activeChannel && (
               <motion.div
                 ...
                 className="flex-1 lg:flex-[2.5] xl:flex-[3] w-full shrink-0 flex flex-col relative z-20 bg-black/50 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border-b lg:border-b-0 lg:border-r border-white/10 h-[55vh] lg:h-full overflow-y-auto"
               >
                 <div className="w-full max-w-[1200px] mx-auto p-4 md:p-6 relative lg:mt-4">
                    {/* Close Button */}
                    <button 
                  ...
                  {/* Channel Info Below Video */}
                    <div className="mt-4 md:mt-5 flex items-center justify-between px-1 md:px-2">
                      <div className="flex items-center gap-4 border border-white/5 bg-white/5 backdrop-blur-md p-3 md:p-4 rounded-2xl w-full">
                          {activeChannel.logoUrl ? (
                             <div className="w-12 h-12 md:w-14 md:h-14 rounded-[10px] md:rounded-xl bg-white/5 border border-white/10 flex items-center justify-center p-2 shadow-inner shrink-0 leading-none">
                               <img src={activeChannel.logoUrl} alt="" className="w-full h-full object-contain drop-shadow-md" />
                             </div>
                          ) : (
                             <div className="w-12 h-12 md:w-14 md:h-14 rounded-[10px] md:rounded-xl bg-white/5 border border-white/10 flex items-center justify-center p-2 shadow-inner shrink-0 leading-none">
                          ...
                          <div className="flex-1">
                             <h2 className="text-lg md:text-2xl font-bold font-bengali text-white/95 leading-none">{activeChannel.name}</h2>
                             <p className="text-[11px] md:text-xs text-white/50 uppercase tracking-wider font-semibold mt-1">
                               {activeChannel.category} Channel
                             </p>
                          </div>
                          <button 
                            onClick={() => toggleFavorite(activeChannel.id)}
                            className={`p-3 shrink-0 rounded-full transition-all duration-300 outline-none ${
                              favorites.includes(activeChannel.id) 
                                ? 'text-[var(--color-accent)] drop-shadow-[0_0_12px_rgba(var(--color-accent-rgb),0.8)]' 
