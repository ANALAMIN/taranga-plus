...
export async function getChannels(): Promise<ChannelFinal[]> {
  try {
    ...
    console.warn('Failed to fetch channels from backend. Using mock data for preview...', error);
    // Return sample data so the UI can be previewed without the Cloudflare backend running
    return [
      {
        id: 'mock-1',
        name: 'Gazi TV (GTV)',
